...
print('Wrote clean build.gradle.kts')
...
stdin, stdout, stderr = ssh.exec_command('cp /opt/SamLogisticApp-signed.apk /var/www/samlogistic/app/SamLogisticApp.apk && docker cp /var/www/samlogistic/app/SamLogisticApp.apk sam-frontend:/usr/share/nginx/html/SamLogisticApp.apk')
stdout.read()

stdin, stdout, stderr = ssh.exec_command('curl -sk -o /tmp/final_verify2.apk -w "HTTP:%{http_code} SIZE:%{size_download}" https://samlogistic.tn/SamLogisticApp.apk && md5sum /tmp/final_verify2.apk')
out = stdout.read().decode('utf-8', errors='replace').strip()
print('\nFinal:')
print(out)
