...
TAR = r'C:\Users\Administrateur\AppData\Local\Temp\SamLogisticApp5.tar'
...
sftp = ssh.open_sftp()
sftp.put(TAR, '/tmp/SamLogisticApp5.tar')
sftp.close()

stdin, stdout, stderr = ssh.exec_command('rm -rf /opt/SamLogisticApp && mkdir -p /opt && tar -xf /tmp/SamLogisticApp5.tar -C /opt/ && rm /tmp/SamLogisticApp5.tar')
stdout.read()

stdin, stdout, stderr = ssh.exec_command('bash -lc "echo sdk.dir=/opt/android-sdk > /opt/SamLogisticApp/local.properties"')
stdout.read()

cmd = 'bash -lc "export ANDROID_HOME=/opt/android-sdk && export PATH=$ANDROID_HOME/cmdline-tools/latest/bin:$ANDROID_HOME/platform-tools:/opt/gradle/gradle-8.6/bin:$PATH && cd /opt/SamLogisticApp && /opt/gradle/gradle-8.6/bin/gradle --no-daemon --refresh-dependencies :app:assembleRelease 2>&1 | tail -120"'
print('>>> Building APK...')
