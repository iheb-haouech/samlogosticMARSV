...
import { Alert, Button, Col, DatePicker, Divider, Flex, Form, Input, InputRef, Row, Tag } from "antd";
...
const UpdateOrderForm = ({ order, onUpdateOrder }: UpdateOrderFormProps) => {
  const { t } = useTranslation();
  const [formValues, setFormValues] = useState<Order>(order);
  const loading = useSelector(selectLoadingState);
  const [tags, setTags] = useState<string[]>(order ? order.refrences : []);
  ...
  const [packagesData, setPackagesData] = useState<PackagesData>({
    ...
    totalWeight: order?.totalWeight || 0, // Add totalWeight initialization
  });
  ...
  useEffect(() => {
    setFormValues({
      ...
      totalWeight: packagesData.totalWeight, // Update totalWeight in formValues
    });
  ...
  const handleSubmit = () => {
    const updatedOrder: Order = {
      ...
      packages: packagesData?.packages.map(({ orderId, updatedAt, createdAt, index, ...pakg }: any) => pakg),
    };
  ...
  const handlePackagesChange = (newPackagesData: PackagesData) => {
    const totalWeight = newPackagesData.packages.reduce((acc, pkg) => acc + (pkg.weight || 0), 0);
    const totalQuantity = newPackagesData.packages.reduce((acc, pkg) => acc + (pkg.quantity || 0), 0);
    ...
    setFormValues({
      ...formValues,
      packages: newPackagesData.packages,
      totalQuantity,
      totalWeight,
    });
  };
                ...
                >
                  <div style={{ marginRight: 10 }}>
                    <DatePicker
                      id='etd'
                      format='YYYY-MM-DD'
                      value={dayjs(formValues?.etd?.split("T")[0])}
                      onChange={(dateString) => {
                        if (dateString) {
                  ...
                  <div style={{ marginRight: 10 }}>
                    <DatePicker
                      id='eta'
                      format='YYYY-MM-DD'
                      value={dayjs(formValues?.eta?.split("T")[0])}
                      onChange={(dateString) => {
                        if (dateString) {
                          const formattedDate = dayjs(dateString).add(1, "hour").toISOString();
                          setFormValues({
